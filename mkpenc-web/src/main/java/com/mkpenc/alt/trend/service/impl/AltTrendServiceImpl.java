package com.mkpenc.alt.trend.service.impl;
...
//import com.mkpenc.alt.status.model.DccGrpTagInfo;
//import com.mkpenc.dcc.tip.service.DccTipService;
import com.mkpenc.alt.trend.mapper.AltTrendMapper;
import com.mkpenc.alt.trend.model.AltSearchTrend;
import com.mkpenc.alt.trend.service.AltTrendService;


@Service("altTrendService")
public class AltTrendServiceImpl implements AltTrendService{
	
	private static Logger logger = LoggerFactory.getLogger(AltTrendService.class);
	...
	@Autowired
	private AltTrendMapper altTrendMapper;
	...
	public List<Map> selectGroupName(Map trendSearchMap) {
		return altTrendMapper.selectGroupName(trendSearchMap);
	}
	
	public String sqlQueryDccReal(AltSearchTrend altSearchTrend) {
		String res = "";
		List<Map> fldNoList = altTrendMapper.selectTrendFldNo(altSearchTrend);
		...
		for( Map fldNo : fldNoList ) {
			...
			List<Map> dccTrendDataList = altTrendMapper.selectTrendData(trendSearchMap);
	...
	public Map getDccValue(AltSearchTrend dccSearchTrend, List<Map> dccGrpTagList, Long lGap, ModelAndView mav) {
	...
	private String setDataConv(int i, String fVal, List<Map> dccGrpTagList, String sMenuNo, Long lGap) {
	    ...
	    if( "DI".equals(strIOType) || "DO".equals(strIOType) ){
	        if( !"".equals(strIOBit) ) {
	            fValue = Double.parseDouble(altCommonService.GetBitVal(fValue+"",strIOBit));
	        }
	...
	public String selectScanTime(AltSearchTrend altSearchTrend){
		List<String> scanTimeList = altTrendMapper.selectScanTime(altSearchTrend);
	...
	public List<Map> getGrpTag(Map searchMap) {
		List<Map> grpTagList = altTrendMapper.selectGrpTagList(searchMap);
		...
		for( Map grpTag : grpTagList ) {
			...
			if( "D".equalsIgnoreCase(grpTag.get("Gubun").toString()) ) {
				...
				if( "1".equals(strFastIOChk) ) {
					ComAltGrpTagInfo dccGrpTagInfo = new ComAltGrpTagInfo();
					dccGrpTagInfo.setHogi(grpTagInfo.get("Hogi").toString());
					dccGrpTagInfo.setADDRESS(grpTagInfo.get("ADDRESS").toString());
					
					String fldNo = basAltOsmsMapper.selectFastIoChk(dccGrpTagInfo);
	...
	public List<Map> rsTrend5sGap(Map trendSearchMap, List<ComTagAltInfo> altGrpTagList, Long lGap, String strFast) {
		...
		get4Map.put("type","3");
		String now_minTime = altTrendMapper.selectMinScantime(get4Map);
		String thistime = "";
		String thistime2 = "";
		...
		while( convDtm(curDate,true).compareTo(convDtm(maxDate,true)) <= 0 ) {
			...
			if( "F".equalsIgnoreCase(strFast) ) {
				...
				for( int ti=1;ti<altGrpTagList.size()+1;ti++ ) {
					Map searchMap = new HashMap();
					
					searchMap.put("hogi",altGrpTagList.get(ti-1).getHogi());
					searchMap.put("startDate",sTime);
					searchMap.put("endDate",eTime);
					searchMap.put("seq","1");
					searchMap.put("type","1");
					
					thistime = altTrendMapper.selectMinScantime(searchMap);
				...
				if( convDtm(now_minTime,true).compareTo(convDtm(eTime,true)) > 0 ) {
					for( int ti=1;ti<altGrpTagList.size()+1;ti++ ) {
						...
						searchMap.put("startDate",sTime);
						searchMap.put("endDate",eTime);
						searchMap.put("seq",altGrpTagList.get(ti-1).getTBLNO());
		...
		if( !"F".equalsIgnoreCase(strFast) ) {
			...
			if( onlyDate.size() > 0 ) {
				...
				for( int vi=1;vi<altGrpTagList.size()+1;vi++ ) {
					Map searchMap = new HashMap();
					
					searchMap.put("hogi",altGrpTagList.get(vi-1).getHogi());
					searchMap.put("startDate",onlyDate2.get(vi).toString());
					searchMap.put("seq",altGrpTagList.get(vi-1).getTBLNO());
					searchMap.put("FldNo",altGrpTagList.get(vi-1).getFLDNO());
					searchMap.put("type","1");
					
					String tmpValue = altTrendMapper.selectTValueTrend(searchMap);
		...
		if( onlyDate2.size() > 0 ) {
			...
			for( int vi=1;vi<altGrpTagList.size()+1;vi++ ) {
				...
				if( "F".equalsIgnoreCase(strFast) ) {
					searchMap.put("seq","1");
					searchMap.put("FldNo",altGrpTagList.get(vi-1).getFLDNO_FAST());
					searchMap.put("type","2");
				} else {
					searchMap.put("seq",altGrpTagList.get(vi-1).getTBLNO());
					searchMap.put("FldNo",altGrpTagList.get(vi-1).getFLDNO());
	...
	public int manageTrendProc(String procBody) {
		return altTrendMapper.manageTrendProc(procBody);
	}
	
	@Override
	public List<Map> callTrendProc(Map procInfo) {
		return altTrendMapper.callTrendProc(procInfo);
	}
	
	@Override
	public int deleteGrpTag(Map trendSearchMap) {
		return altTrendMapper.deleteGrpTag(trendSearchMap);
	}
	
	@Override
	public String selectISeqTagDccSearch(Map trendSearchMap) {
		return altTrendMapper.selectISeqTagDccSearch(trendSearchMap);
	}
	
	@Override
	public int insertGrpTag(Map trendSearchMap) {
		return altTrendMapper.insertGrpTag(trendSearchMap);
	}
	
	@Override
	public List<Map> selectSetIOList(AltSearchTrend altSearchTrend) {
		return altTrendMapper.selectSetIOList(altSearchTrend);
	}
	
	@Override
	public int updateTrendRange(Map trendSearchMap) {
		return altTrendMapper.updateTrendRange(trendSearchMap);
	}
	
	@Override
	public int addGroupTrendFixed(Map trendSearchMap) {
		return altTrendMapper.addGroupTrendFixed(trendSearchMap);
	}
	
	@Override
	public int updateGroupTrendFixed(Map trendSearchMap) {
		return altTrendMapper.updateGroupTrendFixed(trendSearchMap);
	}
	
	@Override
	public List<Map> selectMoveTagTrend(Map trendSearchMap) {
		return altTrendMapper.selectMoveTagTrend(trendSearchMap);
	}
	
	@Override
	public List<Map> selectTrendTagSearch(Map trendSearchMap){
		return altTrendMapper.selectTrendTagSearch(trendSearchMap);
	}
	
	@Override
	public String selectMaxUGrpNo(Map trendSearchMap) {
		return altTrendMapper.selectMaxUGrpNo(trendSearchMap);
	}
	
	@Override
	public int addGroupTrendSpare(Map trendSearchMap) {
		return altTrendMapper.addGroupTrendSpare(trendSearchMap);
	}
	
	@Override
	public int addGrpTagTrendSpare(Map trendSearchMap) {
		return altTrendMapper.addGrpTagTrendSpare(trendSearchMap);
	}
	
	@Override
	public int delGroupTrendSpare(Map trendSearchMap) {
		return altTrendMapper.delGroupTrendSpare(trendSearchMap);
	}

	@Override
	public int delGrpTagTrendSpare(Map trendSearchMap) {
		return altTrendMapper.delGrpTagTrendSpare(trendSearchMap);
	}
	
	@Override
	public List<Map> selectGroupGetTrendSpare(Map trendSearchMap) {
		return altTrendMapper.selectGroupGetTrendSpare(trendSearchMap);
	}
	
	@Override
	public List<Map> selectGrpTagTrendSpare(Map trendSearchMap) {
		return altTrendMapper.selectGrpTagTrendSpare(trendSearchMap);
	}
	
	@Override
	public int insertGrpTagTrendSpare(Map trendSearchMap) {
		return altTrendMapper.insertGrpTagTrendSpare(trendSearchMap);
	}
}
