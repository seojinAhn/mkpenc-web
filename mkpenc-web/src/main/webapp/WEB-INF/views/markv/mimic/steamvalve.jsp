...
var tMarkTagSeq = [	
	...
	${MarkTagInfoList[25].iSeq},${MarkTagInfoList[26].iSeq},${MarkTagInfoList[27].iSeq},${MarkTagInfoList[28].iSeq}
];
...
$(function () {
	...
	$(document.body).delegate('#steamvalve_div label', 'dblclick', function() {		
	...
	$(document.body).delegate('#steamvalve_div div', 'dblclick', function() {		
		var cId = this.id.indexOf('fValue') > -1 ? this.id.substring(4) : this.id;
		if( cId != null && cId != '' && cId != 'undefined' ) {
			showTag(cId,tMarkTagSeq[cId]);
		}
...
function setTimer(hogiHeader,xyHeader,interval) {
	if( interval > 0 ) {
		...
		var	comSubmit	=	new ComSubmit("steamvalveFrm");
		comSubmit.setUrl("/markv/mimic/steamvalve");
		comSubmit.addParam("hogiHeader",hogiHeader);
		comSubmit.addParam("xyHeader",xyHeader);
		comSubmit.submit();
	}
...
function saveTag() {
	...
	comSubmit.addParam("rUrl", 'steamvalve');	//return param 해당파일명 입력
