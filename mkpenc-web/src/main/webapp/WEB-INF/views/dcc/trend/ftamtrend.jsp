...
<script type="text/javascript">
	...
	$(function() {
		...
		$(document.body).delegate('#H', 'click', function() {
			...
			if( !alreadyClicked ) {
				...
				var txtTimeGapI = $("#txtTimeGapI").val() == null ? 5 : $("#txtTimeGapI").val();
				...
				mskSDtm.setSeconds(mskSDtm.getSeconds()-txtTimeGapI*840);
		...
		$(document.body).delegate('#alarmOK','click',function() {
			alarmOK_click();
		});
	...
	function setTimer(num){
		if( num == 0 ) {
			...
			comAjax.addParam('uMenuNo','21');
			comAjax.addParam('fixed','H');
			...
			return new Promise(function(resolve,reject) {
				...
				for( var ci=0;ci<xAxis.length;ci++ ) {
					...
					if( ci == 0 ) {
						...
						var xDataSub = getSub(xData,xAxis[ci],0,0);
						chartConfig = {
							global: {
								svg: {
									classname: 'customClass' // 해당 차트의 svg 태그에 커스텀 클래스 설정
	...
	function checkHPAlarm() {
		if( $("#cboUGrpName option:selected").val() == 100 || $("#cboUGrpName option:selected").val() == '100' ) {
			gHPValue = $("#lblValue0").text()*1;
			...
			if( gHPValue > 3 ) {
				if( !gHPOK ) {
					...
					if( gHPAlarm == 0 && gHPValue > 62 ) {
						gHPOK = false;
						$("#HPAlarm").css("display","");
						alarmTimer = true;
					} else {
						if( gHPAlarm == 1 && gHPValue > 150 ) {
							gHPOK = false;
							$("#HPAlarm").css("display","");
							alarmTimer = true;
						}
	...
	function alarmOK_click() {
		gHPOK = true;
		$("#HPAlarm").css("display","none");
		alarmTimer = false;
	}
	...
	function Timer1() {
		//if( alarmTimer ) {
			...
			if( gHPValue > 150 ) {
				...
				txtMessage = '조기예비경보 발생';
				gHPAlarm = 0;
			}
			$("#lblMessage").text(txtMessage);
			
			if( $("#lblMessage").text().length > 0 ) {
				$("#lblMessage").css('font-color','#e2e2e2');
				if( gHPAlarm >= 0 && !$("input:checkbox[id='chkAlarm']").is(":checked") ) {
					beep();
				...
				$("#lblMessage").css('font-color','red');
			}
		//}
	}
	
	function beep() {
		var snd = new Audio("/resources/alarm/BUZZ.WAV");
	...
	function triggerChart(his) {
		...
		if( selGrpName != 'null' ) {
			...
			comAjax.addParam("fixed",'H');
			comAjax.addParam("sMenuNo",'21');
			comAjax.addParam("sGrpID",'Trend');
	...
	function doHistorical(type,refTime) {
		...
		if( type == 0 ) {
			if( useGap ) {
				mskFSDtm.setSeconds(mskFSDtm.getSeconds()-txtTimeGapI*840);
			}
		} else if( type == 2 ) {
			//if( useGap ) {
				mskFSDtm.setSeconds(mskFSDtm.getSeconds()-txtTimeGapI*840);
			...
			//if( useGap ) {
				mskFEDtm.setSeconds(mskFEDtm.getSeconds()+txtTimeGapI*840);
			//}
		}
		...
		if( type == 0 || type == 2 ) {
			if( useGap ) {
				mskCSDtm.setSeconds(mskCSDtm.getSeconds()-txtTimeGapI*840);
			}
		} else {
			mskCSDtm.setSeconds(mskCSDtm.getSeconds()-txtTimeGapI*840);
		}
	...
	function toCSV() {
		...
		var comSubmit = new ComSubmit("selGrpFrm");
		comSubmit.setUrl("/dcc/trend/ftamExcelExport");
		...
		comSubmit.addParam('sMenuNo','21');
		comSubmit.addParam('sGrpID','Trend');
		comSubmit.addParam('fixed','H');
	...
	function saveRange() {
		...
		for( var i=0;i<tagCnt;i++ ) {
			/*tmp['grpName'] = $("#lblTagName"+i).text().substring(2,$("#lblTagName"+i).text().length);
			tmp['tagNo'] = $("#lblValue"+i).attr('value').split('_')[0];
			tmp['maxVal'] = $("#Hi"+i).text();
			tmp['minVal'] = $("#Low"+i).text();
			tmp['hogi'] = $("#lblValue"+i).attr('value').split('_')[1];
			tmp['grpHogi'] = $("#lblValue"+i).attr('value').split('_')[2];*/
	...
	function toIMG() {
	 	...
	 	doModal().then(function() {
			html2canvas($("#captureArea")[0]).then(canvas => {
				...
				saveAs(canvas.toDataURL(),"ftamTrend.jpg"); //다운로드 되는 이미지 파일 이름 지정
			});
		});
	}
...
<div class="wrap">
	...
	<div class="container">
		...
		<div id="captureArea" class="contents">
			<!-- page_title -->
			<div class="page_title" style="max-height:45px;margin-bottom:10px">
				<h3 id="trendTitle">FTAM TREND</h3>
				<div id="trendPath" class="bc"><span>DCC</span><span>Trend</span><strong>FTAM TREND</strong></div>
			</div>
			<!-- //page_title -->
			<!-- fx_srch_wrap -->
				...
				<div class="fx_srch_form" style="margin-top:-5px">
					<div class="fx_srch_row">
						...
						<div class="fx_srch_item">
							...
							<label>
                            	<input id="txtTimeGapI" type="text" value="5" onkeyup="javascript:setIntervalTime(this.value);">
						...
						<div class="fx_srch_item no_label">	
                            <div class="fx_form">
                                ...
                                <c:forEach var="grpInfo" items="${DccGroupList}">
	                                <c:if test="${grpInfo.groupNo eq 100 or grpInfo.groupNo eq '100'}">
		...
		<!-- //contents -->
                ...
                <div class="loader_wrap" id="modal_loading" style="display:none">
               	...
               	<!-- //Loading -->
                <div id="HPAlarm" style="display:none;position:absolute;top:141px;left:425px;background-color:#e2e2e2">
                	<table>
                		<thead></thead>
                		<tbody>
                			<tr>
                				<td rowspan=2 style="font-size:40px;color:red;padding-left:5px"><label id="lblMessage">조기예보경보 발생</label></td>
                				<td style="height:15px;padding-left:10px;padding-right:5px">
	                				<input id="chkAlarm" type="checkbox" style="margin-right:5px">
	                				<label style="margin-bottom:-5px">소리제거</label>
								</td>
                			</tr>
                			<tr>
                				<td style="height:15px;padding-left:10px">
                					<label id="alarmOK" style="width:50px;height:25px;margin-left:15px;margin-top:-10px" class="btn_list" style="width:45px;margin-left:12px">확인</label>
               					</td>
               				</tr>
                		</tbody>
                	</table>
