...
<script type="text/javascript">
	...
	function setTimer(num){
		if( num == 0 ) {
			...
			comAjax.addParam('uMenuNo','51');
			comAjax.addParam('fixed','A');
		...
		if( selGrpName != 'null' ) {
			...
			for( var ty=1;ty<hVal.length+1;ty++ ) {
				...
				if( ty == 1 ) {
					...
					//axisConfigs.splice(Math.floor(ty/2),1,axisConfig);
				}
				//console.log(ty);
				//console.log(axisConfig);
				//axisConfigs[Math.floor(ty/2)] = axisConfig;
				//axisConfigs.push(axisConfig);
			}
			//console.log(axisConfigs);
					...
					for( var n=0;n<xDataSize;n++ ) {
						if( xDataSize == 1 ) {
							...
							if( dif < gInterval && dif > 0 && n == 0 ) {
								xData2.splice(xData2.length-1,1,tmpXData[n]);
								//xData2.push(tmpXData[n]);
							} else {
								xData2.push(tmpXData[n]);
							}
	...
	function toCSV() {
		...
		var comSubmit = new ComSubmit("selGrpFrm");
		comSubmit.setUrl("/dcc/trend/dsExcelExport");
		...
		comSubmit.addParam('sMenuNo','51');
		comSubmit.addParam('sGrpID','Trend');
		comSubmit.addParam('fixed','A');
		comSubmit.addParam("gHis",his);
		comSubmit.submit();
	}
	
	function openWindowPop() {
		var url = 'realtimetrendfixed';
		var name = 'rttf_pop';
	...
	<div class="container">
		...
		<div id="captureArea" class="contents">
			...
			<div class="page_title" style="max-height:45px;margin-bottom:10px">
				...
				<div id="trendPath" class="bc"><span>DCC</span><span>Performance</span><strong>DCC STATUS</strong></div>
			</div>
			<!-- //page_title -->
			<!-- fx_srch_wrap -->
